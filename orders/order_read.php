...
<?php
    ...
    $filters = date("date_order");

    $queryResult = $db->query(" SELECT orders.id, name_customer, date_order, status FROM orders JOIN customer ON orders.customer_id = customer.id ORDER BY date_order DESC; ");
    $result=array();
